...
static const char *const kPropIdToName[] =
        {
                "0", "1", "2", "Path", "Name", "Extension", "Folder", "Size", "Packed Size",
                "Attributes", "Created", "Accessed", "Modified", "Solid", "Commented", "Encrypted",
                "Split Before", "Split After", "Dictionary Size", "CRC", "Type", "Anti", "Method",
                "Host OS", "File System", "User", "Group", "Block", "Comment", "Position",
                "Path Prefix", "Folders", "Files", "Version", "Volume", "Multivolume", "Offset",
                "Links", "Blocks", "Volumes", "Time Type", "64-bit", "Big-endian", "CPU",
                "Physical Size", "Headers Size", "Checksum", "Characteristics", "Virtual Address",
                "ID", "Short Name", "Creator Application", "Sector Size", "Mode", "Symbolic Link",
                "Error", "Total Size", "Free Space", "Cluster Size", "Label", "Local Name",
                "Provider", "NT Security", "Alternate Stream", "Aux", "Deleted", "Tree", "SHA-1",
                "SHA-256", "Error Type", "Errors", "Errors", "Warnings", "Warning", "Streams",
                "Alternate Streams", "Alternate Streams Size", "Virtual Size", "Unpack Size",
                "Total Physical Size", "Volume Index", "SubType", "Short Comment", "Code Page",
                "Is not archive type", "Physical Size can't be detected", "Zeros Tail Is Allowed",
                "Tail Size", "Embedded Stub Size", "Link", "Hard Link", "iNode", "Stream ID",
                "Read-only", "Out Name", "Copy Link"
        };
...
static void
PrintStringToString(char *dest, EAdjustment adj, unsigned width, const char *textString) {
    unsigned numSpaces = 0;
...
public:
    ...
    HRESULT getSize(UInt32 index, CListStat &st);
        ...
        if (f.IsRawProp) {
            ...
            switch (f.PropID) {
                ...
                case kpidPackSize:
                    ...
                    break;
                case kpidMTime:
                    if (st.MTime.Def) prop = st.MTime.Val;
                    break;
                default: RINOK(Arc->Archive->GetProperty(index, f.PropID, &prop));
            }
            if (f.PropID == kpidAttrib && (prop.vt == VT_EMPTY || prop.vt == VT_UI4)) {
                GetAttribString((prop.vt == VT_EMPTY) ? 0 : prop.ulVal, IsDir, techMode,
                                temp + tempPos);
                if (techMode)
...
HRESULT CFieldPrinter::getSize(UInt32 index, CListStat &st) {
    char temp[128];
    size_t tempPos = 0;
    ...
    */
    FOR_VECTOR (i, _fields) {
        ...
        const unsigned width = f.Width;
//        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "756 %u", f.Width);
        ...
        if (f.IsRawProp) {
            ...
            if (dataSize != 0) {
                ...
                if (needPrint) {
                    ...
                    if (dataSize > kMaxDataSize) {
                        g_StdOut << "data:";
                        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "697 %u", dataSize);

                        return dataSize;
                    } else {
                        char hexStr[kMaxDataSize * 2 + 4];
                        HexToString(hexStr, (const Byte *) data, dataSize);
                        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "703 %s", hexStr);
                        g_StdOut << hexStr;
                    }
                    return dataSize;
                }
            ...
            if (f.PropID == kpidAttrib && (prop.vt == VT_EMPTY || prop.vt == VT_UI4)) {
                ...
                if (techMode) {
                    g_StdOut << temp + tempPos;
                    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "732 %zu", tempPos);
                } else
                    tempPos += strlen(temp + tempPos);
                __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "734 %zu", tempPos);
                ...
                if (techMode) {
                    g_StdOut << temp + tempPos;
                    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "745 %zu", tempPos);
                } else {
                    size_t len = strlen(temp + tempPos);
                    tempPos += len;
                    if (len < (unsigned) f.Width) {
                    ...
                    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "753 %u", f.Width);
                    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "753 %u", len);
                    return len;
                }
            } else if (prop.vt == VT_BSTR) {
                TempWString.SetFromBstr(prop.bstrVal);
                ...
                char s[64];
                ConvertPropertyToShortString(s, prop, f.PropID);
                return static_cast<LONG>(st.Size.Val);
                __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "770 %u",
                                    prop.filetime.dwHighDateTime);
                if (techMode) {
                    g_StdOut << s;
                    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "770 1 %s", s);
                } else {
                    PrintStringToString(temp + tempPos, f.TextAdjustment, width, s);
                    tempPos += strlen(temp + tempPos);
                }
                __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "770 2 %zu", tempPos);
                return tempPos;
            }
        }
        if (techMode)
...
static void PrintPropertyPair2(CStdOutStream &so, PROPID propID, const wchar_t *name,
                               const CPropVariant &prop) {
    UString s;
...
static HRESULT
PrintArcProp(CStdOutStream &so, IInArchive *archive, PROPID propID, const wchar_t *name) {
    CPropVariant prop;
...
HRESULT
Print_OpenArchive_Props(CStdOutStream &so, const CCodecs *codecs, const CArchiveLink &arcLink) {
...
HRESULT
Print_OpenArchive_Error(CStdOutStream &so, const CCodecs *codecs, const CArchiveLink &arcLink) {
#ifndef _NO_CRYPTO
...
#endif
    {
        if (arcLink.NonOpen_ErrorInfo.ErrorFormatIndex >= 0) {
            ...
            PrintArcTypeError(so, codecs->Formats[arcLink.NonOpen_ErrorInfo.ErrorFormatIndex].Name,
                              false);
        } else
...
//        std::wofstream os("/storage/emulated/0/test.dat", std::ios::out | std::ios::binary);
        ...
        for (UInt32 i = 0; i < numItems; i++) {
            ...
            if (fp.IsDir) {
                fwrite("\"dir\":\"", sizeof("\"dir\":\""), 1, outFile);
            } else {
