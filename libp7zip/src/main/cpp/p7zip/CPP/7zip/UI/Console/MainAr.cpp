...
)
{
  ...
  catch(const CSystemException &systemError)
  {
    ...
    {
      PrintError(kMemoryExceptionMessage);
      return const_cast<char *>("Memory error");
    }
    if (systemError.ErrorCode == E_ABORT)
    {
      PrintError(kUserBreakMessage);
    ...
    return const_cast<char *>("FatalError");
  }
  catch(NExitCode::EEnum &exitCode)
  {
    FlushStreams();
    if (g_ErrStream)
      *g_ErrStream << kInternalExceptionMessage << exitCode << endl;
    return const_cast<char *>("" + exitCode);
  }
  catch(const UString &s)
  {
    if (g_ErrStream)
  ...
  return const_cast<char *>("NO EXCEPTION");
}
